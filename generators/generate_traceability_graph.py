...
OUTPUT_RELATIVE_PATH = Path("domains") / "TRACEABILITY.md"
...
def render_graph(snapshot: RepositoryModel) -> str:
    ...
    external_node_ids = {
        target_id: f"x{index}" for index, target_id in enumerate(external_targets)
    }
    ...
    for record, artifact_type in records:
        ...
        out.append(f'    {node_ids[document_id]}["{document_id}"]:::{style_class}')

    if external_targets:
        out.append("    %% External Relationship Targets")
        for target_id in external_targets:
            out.append(f'    {external_node_ids[target_id]}["{target_id}"]:::external')
...
def generate_graph(
    ...
    # and STD-only repositories must not manufacture an empty domains tree.
    has_domain_corpus = any(
        artifact_type in {"PAD", "SAD"} for _, artifact_type in records
    )
