...
def generate_index(
    ...
    if not records:
        print("[SKIP] No ADR corpus; decisions/INDEX.md not generated")
        return None

    output_dir = root / ADR_ROOT
    if not output_dir.is_dir():
        raise RepositoryModelError(
            f"ADR corpus exists but expected layer directory '{output_dir}' is missing."
        )
    ...
    print(f"[OK] Generated ADR Index ({len(records)} records) -> {output_path}")
    return output_path
