...
spec = importlib.util.spec_from_file_location(
    "github_app_preflight", PACKAGE / "github_app_preflight.py"
)
...
class HelperFixture:
    ...
    def setUpClass(cls):
        ...
        cls.pem = cls.rsa_key.private_bytes(
    ...
    def setUp(self):
        ...
        self.install = {
            "id": self.config.installation_id,
            "app_id": self.config.app_id,
            "account": {"login": "scnehaux"},
            "permissions": dict(target.REQUIRED_PERMISSIONS),
            "suspended_at": None,
        ...
        guard = patch.object(
            target,
            "build_opener",
            side_effect=AssertionError("Live HTTP is forbidden in offline tests"),
        )
        guard.start()
        ...
        runtime = patch.object(
            target,
            "__file__",
            str(self.root / "trusted-kit" / "github_app_preflight.py"),
        )
        runtime.start()
...
class PreflightTests(HelperFixture, unittest.TestCase):
    ...
    def test_config_schema_version(self):
        for version in (True, 2, "1", None):
            self.config_file.write_text(
                json.dumps(dict(self.raw, config_version=version))
            )
    ...
    def test_reject_encrypted_key(self):
        self.key.write_bytes(
            self.rsa_key.private_bytes(
                serialization.Encoding.PEM,
                serialization.PrivateFormat.PKCS8,
                serialization.BestAvailableEncryption(b"dummy-test-password"),
            )
        )
        self.fail_code("key-invalid", target.make_jwt, self.key, self.config.client_id)
    ...
    def test_reject_world_readable_key(self):
        ...
        self.fail_code(
            "key-permissions", target.make_jwt, self.key, self.config.client_id
        )
    ...
    def test_reject_key_in_repository_or_git_worktree(self):
        for worktree in (False, True):
            ...
            if worktree:
                ...
                git.mkdir()
            self.fail_code(
                "key-location", target.make_jwt, self.key, self.config.client_id
            )

    def test_reject_key_inside_setup_directory(self):
        with patch.object(
            target, "__file__", str(self.root / "github_app_preflight.py")
        ):
            self.fail_code(
                "key-location", target.make_jwt, self.key, self.config.client_id
            )
    ...
    def test_preflight_success_is_not_governance_success(self):
        ...
        self.assertEqual(
            ...
            [
                "/app",
                "/app/installations/159870521",
                "/repos/scnehaux/codex/installation",
            ],
        )
    ...
    def test_wrong_app_fails_before_installation_lookup(self):
        self.fail_code(
            ...
            self.config,
            "dummy",
            lambda path, token: {"id": 123},
        )

    def test_installation_identity_owner_status_and_scope(self):
        cases = [
            ({"id": 123}, "installation-mismatch"),
            ({"id": True}, "installation-mismatch"),
            ({"app_id": 123}, "installation-app-mismatch"),
            ({"app_id": "4864946"}, "installation-app-mismatch"),
            ({"account": None}, "installation-owner"),
            ({"account": {"login": "someone-else"}}, "installation-owner"),
            ({"suspended_at": "2026-01-01"}, "installation-suspended"),
        ...
        for update, code in cases:
            with self.subTest(update=update):
                self.fail_code(
                    code,
                    target.verify_installation,
                    dict(self.install, **update),
                    self.config,
                )
        ...
        for field, expected in target.REQUIRED_PERMISSIONS.items():
            for value in (None, "none", "write" if expected == "read" else "read"):
                with self.subTest(field=field, value=value):
                    ...
                    self.fail_code(
                        "permissions-mismatch", target.verify_permissions, data
                    )
        self.fail_code(
            "permissions-excess",
            target.verify_permissions,
            dict(target.REQUIRED_PERMISSIONS, administration="write"),
        )
    ...
    def test_repo_installation_mismatch_is_not_accepted(self):
        ...
        self.fail_code(
            "installation-mismatch", target.preflight, self.config, "DUMMY-JWT", read
        )
    ...
    def test_redirect_handler_does_not_forward_credentials(self):
        ...
        self.assertIsNone(
            handler.redirect_request(None, None, 302, "", {}, "https://elsewhere.test/")
        )
    ...
    def test_http_errors_are_sanitized(self):
        for status in (301, 302, 307, 401, 403, 404, 422, 500):
            error = HTTPError(
                ...
                "SENSITIVE-UPSTREAM",
                {},
                io.BytesIO(b"DO-NOT-PRINT-BODY"),
            )
            with patch.object(target, "build_opener") as factory:
    ...
    def test_network_errors_are_sanitized(self):
        for error in (
            URLError("PRIVATE-INTERNAL"),
            TimeoutError("PRIVATE-INTERNAL"),
            OSError("PRIVATE-INTERNAL"),
        ):
            with patch.object(target, "build_opener") as factory:
                factory.return_value.open.side_effect = error
    ...
    def test_malformed_api_responses_fail_closed(self):
        cases = [
            ...
            (b"[]", "api-shape"),
            (b"null", "api-shape"),
            (b"x" * (target.MAX_RESPONSE_BYTES + 1), "api-response-size"),
        ]
    ...
    def test_cli_json_and_text_do_not_render_token(self):
        ...
        for flags in ([], ["--json"]):
            output = io.StringIO()
            with (
                patch.object(target, "make_jwt", return_value="DO-NOT-PRINT-TOKEN"),
                patch.object(target, "preflight", return_value=report),
    ...
    def test_cli_known_error_is_nonzero(self):
        ...
        with (
            patch.object(
                target,
                "make_jwt",
                side_effect=target.PreflightError("key-invalid", "Fixed safe error"),
            ),
            redirect_stdout(output),
        ):
    ...
    def test_cli_unknown_error_masks_sensitive_details(self):
        ...
        with (
            patch.object(
                target, "make_jwt", side_effect=RuntimeError("SECRET-TOKEN-OR-PEM")
            ),
            redirect_stdout(output),
        ):
    ...
    def test_default_key_paths(self):
        with patch.object(
            target,
            "os",
            SimpleNamespace(name="nt", environ={"LOCALAPPDATA": str(self.root)}),
        ):
            self.assertEqual(
                target.default_key_path(),
                self.root / "scnehaux-codex-authority/secrets/github-app.pem",
            )
        with (
            patch.object(target, "os", SimpleNamespace(name="posix")),
            patch.object(target.Path, "home", return_value=self.root),
        ):
            self.assertEqual(
                target.default_key_path(),
                self.root
    ...
    def test_key_read_error_and_missing_dependencies(self):
        with patch.object(
            type(self.key), "open", side_effect=PermissionError("SECRET")
        ):
            self.fail_code(
                "key-unreadable", target.make_jwt, self.key, self.config.client_id
            )
        with patch.dict(sys.modules, {"jwt": None}):
            self.fail_code(
                "dependencies-missing", target.make_jwt, self.key, self.config.client_id
            )
        with patch.object(jwt, "encode", return_value=b"bad-token"):
            self.fail_code(
                "key-invalid", target.make_jwt, self.key, self.config.client_id
            )
    ...
    def test_extra_permissions_none_is_allowed(self):
        target.verify_permissions(
            dict(target.REQUIRED_PERMISSIONS, administration="none")
        )
    ...
    def test_transport_rejects_path_traversal_and_other_writes(self):
        ...
        for method, path in (
            ("PATCH", "/repos/scnehaux/codex/check-runs/1"),
            ("POST", "/repos/scnehaux/codex/git/refs"),
            ("DELETE", "/repos/scnehaux/codex"),
        ):
            self.fail_code(
                "api-method", target.request_json, path, "dummy", method=method
            )
        self.fail_code("api-body", target.request_json, "/app", "dummy", body={})
        self.fail_code(
            "probe-payload",
            target.request_json,
            "/repos/scnehaux/codex/check-runs",
            "dummy",
            method="POST",
            body={"name": "Codex Governance Authority", "conclusion": "success"},
        )
    ...
    def test_post_and_delete_transport(self):
        for path, method, status, raw, body in (
            ...
            (
                ...
                201,
                b'{"id":1}',
                {"name": target.PROBE_NAME, "conclusion": "neutral"},
            ),
    ...
    def test_cli_interrupt_and_plain_known_error(self):
        for error, code in (
            ...
            (target.PreflightError("example", "Safe fixed error"), 1),
        ):
            with (
                patch.object(target, "make_jwt", side_effect=error),
...
class ProbeTests(HelperFixture, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.listing = {
            "total_count": 1,
            "repositories": [{"full_name": "scnehaux/codex"}],
        }
        ...
        self.check = {
            ...
            "head_sha": self.sha,
            "name": target.PROBE_NAME,
            "status": "completed",
            "conclusion": "neutral",
            "external_id": "codex-connectivity/" + self.sha,
        }
    ...
    def request(self, path, token, *, method="GET", body=None):
        ...
        if path == self.fail_at:
            raise target.PreflightError(
                "simulated", "Fixed error without upstream data"
            )
        if path == "/app":
            return {"id": self.config.app_id}
    ...
    def run_probe(self):
        return target.run_probe(
            self.config, "SYNTHETIC-APP-JWT", self.sha, self.request
        )
    ...
    def test_cli_preview_never_reads_credentials_or_calls_api(self):
        output = io.StringIO()
        with (
            patch.object(target, "make_jwt") as make,
            patch.object(target, "request_json") as request,
            redirect_stdout(output),
        ):
            code = target.main(
                ["--config", str(self.config_file), "--probe-sha", self.sha]
            )
            make.assert_not_called()
    ...
    def test_missing_or_incorrect_write_confirmations_are_credential_free(self):
        ...
        for flags in cases:
            with (
                ...
                self.assertEqual(
                    target.main(["--config", str(self.config_file), *flags]), 1
                )
                make.assert_not_called()
    ...
    def test_probe_success_exact_scope_readback_and_revocation(self):
        ...
        self.assertEqual(
            sum(
                m == "POST" and p.endswith("/check-runs")
                for m, p, _, _ in self.api_calls
            ),
            1,
        )
    ...
    def test_failures_after_issuance_always_revoke_no_retries(self):
        for path in (
            ...
            f"/repos/scnehaux/codex/commits/{self.sha}",
            "/repos/scnehaux/codex/check-runs",
            "/repos/scnehaux/codex/check-runs/42",
        ):
            self.fail_at = path
            self.api_calls.clear()
    ...
    def test_cli_write_output_and_failure_never_print_secrets(self):
        ...
        output = io.StringIO()
        with (
            patch.object(target, "make_jwt", return_value="SECRET-JWT"),
            patch.object(target, "request_json", side_effect=self.request),
            redirect_stdout(output),
        ):
