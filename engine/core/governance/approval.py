...
class ApprovalPackage:
    ...
    def __post_init__(self) -> None:
        ...
        if self.policy_id is not None:
            object.__setattr__(
                self, "policy_id", _required(self.policy_id, "policy_id")
            )
...
class ApprovalDecision:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self, "decision_id", _required(self.decision_id, "decision_id")
        )
        ...
        object.__setattr__(
            self, "authority_type", _required(self.authority_type, "authority_type")
        )
        ...
        if self.exception_ref is not None:
            object.__setattr__(
                self, "exception_ref", _required(self.exception_ref, "exception_ref")
            )
        if self.outcome is DecisionOutcome.APPROVED:
            if not self.package.mechanically_eligible and self.exception_ref is None:
                raise ValueError(
                    "approval with mechanical blockers requires exception_ref"
                )
            if (
                self.authority_type == "policy"
                and not self.package.auto_approval_eligible
            ):
                raise ValueError(
                    "policy approval requires auto_approval_eligible package"
                )
