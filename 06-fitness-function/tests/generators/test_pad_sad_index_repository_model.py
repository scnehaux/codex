...
def test_pad_only_generates_only_domain_index(tmp_path):
    ...
    assert generated == (tmp_path / "03-domain" / "INDEX.md",)
    assert not (tmp_path / "04-system").exists()
...
def test_sad_only_generates_only_system_index(tmp_path):
    ...
    _write_artifact(
        tmp_path / "04-system" / "system" / "SAD-001.md",
        "SAD-001",
    ...
    assert generated == (tmp_path / "04-system" / "INDEX.md",)
    assert not (tmp_path / "03-domain").exists()
...
def test_rerun_is_deterministic_and_generated_indexes_are_not_reingested(tmp_path):
    ...
    _write_artifact(
        tmp_path / "03-domain" / "z" / "PAD-002.md",
        "PAD-002",
    )
    _write_artifact(
        tmp_path / "03-domain" / "a" / "PAD-001.md",
        "PAD-001",
    )
