...
def test_control_and_intelligence_do_not_import_each_other():
    for path in (ROOT / "engine" / "control").rglob("*.py"):
        assert not any(
            item.startswith("engine.intelligence") for item in imports(path)
        ), path
    for path in (ROOT / "engine" / "intelligence").rglob("*.py"):
        assert not any(item.startswith("engine.control") for item in imports(path)), (
            path
        )
...
def test_approval_contract_is_neutral_core():
    ...
    assert not any(
        item.startswith("engine.control") or item.startswith("engine.intelligence")
        for item in imports(path)
    )
