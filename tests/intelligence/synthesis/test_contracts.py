...
from engine.intelligence.synthesis import ArchitectureProposal, ArtifactDraft, DraftPayload
...
def test_architecture_proposal_validates_types_and_lineage(
    intent, architecture_plan, context_package, proposed_claim, declared_claim, research_plan
):
    with pytest.raises(TypeError, match="IntentSpec"):
        ArchitectureProposal("p", "intent", architecture_plan, context_package, (proposed_claim,))
    ...
    with pytest.raises(ValueError, match="proposal intent"):
        ArchitectureProposal("p", other_intent, architecture_plan, context_package, (proposed_claim,))
    with pytest.raises(TypeError, match="ContextPackage"):
        ArchitectureProposal("p", intent, architecture_plan, "context", (proposed_claim,))
    with pytest.raises(ValueError, match="must not be empty"):
        ArchitectureProposal("p", intent, architecture_plan, context_package, ())
    with pytest.raises(TypeError, match="Claim"):
        ArchitectureProposal("p", intent, architecture_plan, context_package, ("claim",))
    with pytest.raises(ValueError, match="must be PROPOSED"):
        ArchitectureProposal("p", intent, architecture_plan, context_package, (declared_claim,))
    with pytest.raises(ValueError, match="unique"):
        ArchitectureProposal("p", intent, architecture_plan, context_package, (proposed_claim, proposed_claim))
    ...
    with pytest.raises(ValueError, match="derived_from lineage"):
        ArchitectureProposal("p", intent, architecture_plan, context_package, (no_lineage,))
    ...
    with pytest.raises(ValueError, match="context/research claims"):
        ArchitectureProposal("p", intent, architecture_plan, context_package, (unknown_lineage,))
    ...
    with pytest.raises(ValueError, match="proposal plan"):
        ArchitectureProposal("p", intent, architecture_plan, context_package, (proposed_claim,), other_research)
    with pytest.raises(TypeError, match="ResearchPackage"):
        ArchitectureProposal("p", intent, architecture_plan, context_package, (proposed_claim,), "research")


def test_architecture_proposal_accepts_matching_research(
    intent, architecture_plan, context_package, proposed_claim, research_plan, declared_claim
):
...
def test_artifact_draft_preserves_proposal_lineage(
    ...
    draft = ArtifactDraft(
        "d", "SAD", proposal, Payload("structured"), (proposed_claim.claim_id,), "SAD-001"
    )
...
def test_artifact_draft_rejects_invalid_payload_and_lineage(
    ...
    with pytest.raises(ValueError, match="unique"):
        ArtifactDraft("d", "SAD", proposal, Payload("x"), (proposed_claim.claim_id, proposed_claim.claim_id))
