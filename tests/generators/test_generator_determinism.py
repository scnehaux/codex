...
def test_traceability_render_is_independent_of_snapshot_order():
    ...
    records = (
        entry("systems/SAD-002.md", "SAD-002", parent_pad="PAD-PLT-002"),
        entry("enterprise/EAD-002.md", "EAD-002", governed_by="GDC-000"),
        entry(
            "domains/PAD-PLT-002.md",
            "PAD-PLT-002",
            realizes_capability=["EAD-002", "EAD-001"],
        ),
        entry("enterprise/EAD-001.md", "EAD-001", governed_by="GDC-000"),
    )
...
def test_topography_uses_live_canonical_inputs_not_git_index(tmp_path, monkeypatch):
    ...
    (tmp_path / "engine" / "control" / "reporting" / "reporter.py").write_text(
        "", encoding="utf-8"
    )
    (tmp_path / "generators" / "scratch").mkdir(parents=True)
    (tmp_path / "generators" / "scratch" / "tmp.py").write_text("", encoding="utf-8")
    (tmp_path / "generators" / "scnehaux_linter.egg-info").mkdir()
    (tmp_path / "generators" / "scnehaux_linter.egg-info" / "PKG-INFO").write_text(
        "", encoding="utf-8"
    )
