...
GENERATOR_PATH = ROOT / "generators" / "generate_traceability_graph.py"
...
def test_traceability_uses_canonical_relationship_fields(tmp_path):
    ...
    _write_artifact(
        tmp_path / "enterprise" / "EAD-0enterprise.md",
        "EAD-001",
    )
    _write_artifact(
        tmp_path / "domains" / "PAD-001-platform.md",
        "PAD-001",
        "  realizes_capability: [EAD-001]\n",
    )
    _write_artifact(
        tmp_path / "systems" / "SAD-001-system.md",
        "SAD-001",
        "  parent_pad: PAD-001\n",
    )
    ...
    assert output == tmp_path / "domains" / "TRACEABILITY.md"
