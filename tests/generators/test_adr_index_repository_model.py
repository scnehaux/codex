...
GENERATOR_PATH = ROOT / "generators" / "generate_adr_index.py"
...
def test_adr_corpus_generates_expected_index(tmp_path):
    ...
    _write_artifact(
        tmp_path / "decisions" / "domain" / "ADR-002.md",
        "ADR-002",
    ...
    assert output == tmp_path / "decisions" / "INDEX.md"
...
def test_render_rejects_record_outside_decision_layer():
    ...
    record = RepositoryAssembler.artifact_from_metadata(
        ...
        source_path="systems/ADR-001.md",
    )
