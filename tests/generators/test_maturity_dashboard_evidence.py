...
from engine.control.repository import RepositoryIngestionError, RepositoryModelError


ROOT = Path(__file__).resolve().parents[2]
GENERATOR_PATH = ROOT / "generators" / "generate_maturity_dashboard.py"
...
def test_approved_artifact_does_not_create_verified_evidence(tmp_path):
    ...
    _write_control_registry(
        tmp_path / "governance" / "normative-control-registry.yaml",
        [
            {
        ...
        [
            {
                ...
                "test_evidence": ["tests/engine/test_cli.py"],
            },
            {
