...
def test_topography_live_paths_filters_and_normalizes(tmp_path, monkeypatch):
    ...
    (tmp_path / "engine" / "control" / "__pycache__" / "x.py").write_text(
        "", encoding="utf-8"
    )
    (tmp_path / "generators" / "scratch").mkdir(parents=True)
    (tmp_path / "generators" / "scratch" / "tmp.py").write_text("", encoding="utf-8")
    (tmp_path / "generators" / "pkg.egg-info").mkdir()
    (tmp_path / "generators" / "pkg.egg-info" / "PKG-INFO").write_text(
        "", encoding="utf-8"
    )
...
def test_rules_formatting_and_global_config():
    ...
    assert (
        module.format_type({"type": "array", "items": {"type": "string"}})
        == "array[string]"
    )
    assert (
        module.format_type({"type": ["string", "array"], "items": {"type": "integer"}})
...
def test_rules_current_block_and_injection_contracts(tmp_path):
    ...
    assert (
        module.get_target_doc_name({"config": {"target_doc": "GDC-001.md"}})
        == "GDC-001.md"
    )
