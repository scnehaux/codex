...
This document defines the **Qualitative Evaluation Criteria** for all architectural artifacts. While other documents in the `governance` suite define _what_ sections must exist or _how_ to audit them, this Framework defines _the rigorous benchmark_ the content must meet to pass the "Quality Gate" before entering the Scnehaux knowledge base.

### The `governance` Ecosystem (Separation of Concerns)
